...
RUN apk add --update apache2 apache2-dev apache2-mod-wsgi build-base python python-dev py-pip \
  && pip install --no-cache-dir --upgrade pip \
  && pip install --no-cache-dir -r ./application/requirements.txt \
  && rm -rf /var/cache/apk/*
  && cp /var/www/apache-flask/application/apache/* /etc/apache2/sites-available/ \
  && a2ensite apache-flask apache-flask-ssl \
  && a2enmod rewrite ssl \
